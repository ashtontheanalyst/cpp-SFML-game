...
void Game::initVariables() {
    ...
    this->enemySpawnTimerMax = 25.f; // Making this smaller means enemies spawn faster, larger number is the opp.
...
// Import the ttf file for our text
void Game::initFonts() {
     // make sure the path is right and accessible
    if (this->font.loadFromFile("/home/dementus/code/cpp/cpp-SFML-tutorials/RobotoCondensed-VariableFont_wght.ttf")) {
        std::cout << "ERROR in INIT: Failed to load font..." << '\n';
    }
}


// Initialize our text and what it default is
void Game::initText() {
    this->uiText.setFont(this->font); // sets the font for our text object
    this->uiText.setCharacterSize(35); // font size
    this->uiText.setFillColor(sf::Color::White);
    this->uiText.setString("ERROR: Not Set"); // Default, will be changed in game
}


void Game::initEnemies() {
    this->enemy.setPosition(sf::Vector2f(20.f, 20.f)); // Sets the def. position of the enemy
...
// Spawns our enemies and sets random types, colors, positions, and adds them to the enemies vector
void Game::spawnEnemy() {
    ...
    int type = rand() % 4; // from 0 to 4, so 5 total types
...
void Game::updateText() {
    std::stringstream ss; // initialize a string stream

    // this formatting is super nice, follows what it'll look like on screen
    ss << "Points: " << this->points << '\n' 
       << "Health: " << this->health;

    this->uiText.setString(ss.str()); // set the string as the stream in string format
}
...
void Game::updateEnemies() {
    ...
    for (int i = 0; i < this->enemies.size(); i++) {
        this->enemies[i].move(0.f, 7.f); // moving them downward at a rate of 5 pixels/second
        ...
        if (this->mouseHeld == false) {
            ...
            for (int i = 0; i < this->enemies.size() && deleted == false; i++) {
                ...
                if (this->enemies[i].getGlobalBounds().contains(this->mousePosView)) {
                    ...
                    // Conditional amount of points based on enemy type (color)
                    if (this->enemies[i].getFillColor() == sf::Color::Red) {
                        this->points += 100; // hardest enemy
                    }
...
void Game::update() {
    ...
    if (this->endGame == false) {
        ...
        this->updateText();           // get the text for the backend
...
// Puts the text on the screen, the RenderTarget is a param. for a standard window to draw or put stuff on
...
void Game::render() {
    ...
    */
    ...
    this->renderEnemies(*this->window); // draw the enemies on the screen
    this->renderText(*this->window);    // put the text on screen
    this->window->display();
}