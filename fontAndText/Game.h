...
#include <sstream> // basically allows us to make a long string with a bunch of data types in it
...
class Game {
    ...
    // These are private because no one really needs to use them except in here, defined in Game.cpp
        ...
        void initVariables();
        void initWindow();
        void initFonts(); // normally used for if you have a ton of fonts to load in, can be used for one also though
        void initText();
        void initEnemies();
    ...
    public:
        ...
        void renderText(sf::RenderTarget &target); // basically just passing it something to draw onto instead of main window
